...
if not (sys.version_info[0] == 2 or platform.architecture()[0]=='32bit'):
    ...
    from yyskmultilearn.ext.keras import Keras

    from yyskmultilearn.problem_transform import BinaryRelevance, LabelPowerset
    from yyskmultilearn.tests.classifier_basetest import ClassifierBaseTest
    ...
    class KerasTest(ClassifierBaseTest):
        ...
        def test_if_sparse_classification_works_on_non_dense_base_classifier(self):
            classifier = BinaryRelevance(classifier=Keras(create_model_single_class, False, KERAS_PARAMS), require_dense=[True, True])
        ...
        def test_if_sparse_classification_works_on_dense_base_classifier(self):
            classifier = LabelPowerset(classifier=Keras(create_model_multiclass, True, KERAS_PARAMS), require_dense=[True, True])
